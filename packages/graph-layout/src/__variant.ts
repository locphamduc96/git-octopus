 ...
 */
export function layoutCommitsVariant(listCommits: Commit[]): GraphRow[] {
	...
	for (const commit of listCommits) {
		...
		for (let i = 0; i < lanes.length; i++) {
			...
			if (demoted) nodeColumn = i;
			if (console.__dbg) console.log('converge', hash.slice(0,7), 'cand', i, 'offshoots', JSON.stringify(laneOffshoots), 'awaits', JSON.stringify(laneAwaits));
		}
